        ...
        <form class="form-horizontal" role="form" method="POST"
            ...
            @if(!$user)
                ...
                <div class="form-group">
                    <label for="password" class="col-md-4 control-label">Konfirmasi Password <span
            ...
            @else
                @set('roles_ids', $user->roles->pluck('id')->toArray())
            @endif
            <div class="form-group">
                ...
                <div class="col-md-6">
                    <select id="role" name="roles[]" class="form-control" required multiple>
                        @foreach($roles as $role)
                            <option value="{{$role->id}}" {{$user && in_array($role->id, $roles_ids) ? 'selected' : ''}}>{{$role->display_name}}</option>
                        @endforeach
                        @if ($errors->has('roles'))
